...
@section('title', 'Edit - About Us Directories Messages -')

@section('content')
    <div class="example">
        <h5>Edit <span class="text-success">{{ $editAboutMessage->heading }}</span> - Click here to About Us Directories Messages
        ...
        <div class="tab-content rounded-bottom">
            <div class="tab-pane p-3 active preview" role="tabpanel" id="preview-824">
                <!-- Contact form start -->
                <form method="POST" action="{{ route('about-message.update', $editAboutMessage->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-8">
                            <input class="form-control @error('heading') is-invalid @enderror" name="heading" id="heading" value="{{ $editAboutMessage->heading }}" type="text" placeholder="Update your About Us Directories Messages Heading here" />
                            @error('heading')
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-8">
                            <textarea name="message" placeholder="Update your About Us Directories Messages here" class="form-control @error('message') is-invalid @enderror" id="message">{{ $editAboutMessage->message }}</textarea>
                            @error('message')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
