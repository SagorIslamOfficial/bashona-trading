...
@section('content')
    <div class="example">
        <h5>Edit
            <span class="text-info">{{ $editTeamCategory->name }}</span> - Click here to all Team Category
        ...
        <div class="tab-content rounded-bottom">
            <div class="tab-pane p-3 active preview" role="tabpanel" id="preview-824">
                <!-- Contact form start -->
                <form method="POST" action="{{ route('team-category.update', $editTeamCategory->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-8">
                            <input class="form-control @error('name') is-invalid @enderror" name="name" id="name" value="{{ $editTeamCategory->name }}" type="text" placeholder="Update your team Category Name. Example: Left or Right side" />
                            @error('name')
