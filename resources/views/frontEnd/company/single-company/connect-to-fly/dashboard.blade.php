...
@section('title', 'Connect To Fly | Company -')
                    ...
                    <div class="row">
                        <div class="col-md-12 align-self-center p-static order-2 text-center">
                            <div class="overflow-hidden pb-2">
                                ...
                                <p class="pt-3">{{ $connectToFlyHeadingAndSubText->subTextConnectToFlyCompany }}</p>
                            </div>
                        </div>
                        <div class="col-md-12 align-self-center order-1">
                            <ul class="breadcrumb d-block text-center appear-animation animated fadeIn appear-animation-visible" data-appear-animation="fadeIn" data-appear-animation-delay="300" style="animation-delay: 300ms;">
                                ...
                                <li class="active">Connect To Fly</li>
                            </ul>
                        </div>
                    </div>
                </div>
        ...
        <div class="container">
            <div id="custom-padding" class="row align-items-center justify-content-center justify-content-lg-between pb-lg-0">
                <div class="col-lg-5 order-2 order-lg-1 pt-4 pt-lg-0 pb-5 pb-lg-0 mt-5 mt-lg-0 appear-animation animated fadeInRightShorter appear-animation-visible" data-appear-animation="fadeInRightShorter" data-appear-animation-delay="200" style="animation-delay: 200ms;">
                    <h2 class="font-weight-bold text-color-black text-7 mb-2 about_us_custom">{{ $companyConnectToFlyAbout->title }}</h2>
                    <p class="lead font-weight-regular text-color-black text-4 text-justify">{!! nl2br(e($companyConnectToFlyAbout->text)) !!}</p>
                </div>
                <div class="col-9 offset-lg-1 col-lg-5 order-1 order-lg-2">
                    <img class="img-fluid box-shadow-3 my-2 border-radius" src="{{ asset('storage/company/all-company/connect-to-fly/about/' . $companyConnectToFlyAbout->image) }}" alt="{{ $companyConnectToFlyAbout->title }}">
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="row justify-content-center pt-5 mt-5">
            <div class="col-lg-9 text-center">
                <div class="appear-animation" data-appear-animation="fadeInUpShorter">
                    <h2 class="font-weight-bold mb-2">{{ $connectToFlyHeadingAndSubText->headingConnectToFlyCompanyService }}</h2>
                    <p class="mb-4">{{ $connectToFlyHeadingAndSubText->subTextConnectToFlyCompanyService }}</p>
                </div>
            </div>
        </div>
        <div class="container py-2 mt-4 pb-5">
            <!-- Services Name Start -->
            <ul class="nav nav-pills sort-source sort-source-style-3 justify-content-center" data-sort-id="connectToFlyCompanyService" data-option-key="filter" data-plugin-options="{'layoutMode': 'fitRows', 'filter': '.{{ $cctfsc->slug }}'}">

                @foreach($companyConnectToFlyServiceCategories as $key => $companyConnectToFlyServiceCategory)
                    <li @if ($key == 0) class="nav-item active" @else class="nav-item" @endif data-option-value=".{{ $companyConnectToFlyServiceCategory->slug }}"><a class="nav-link text-1 text-uppercase" href="#">{{ $companyConnectToFlyServiceCategory->name }}</a></li>
                @endforeach

            </ul>
            <!-- Services Name End -->
            <div class="sort-destination-loader mt-4 pt-2 sort-destination-loader-loaded">
                <div class="row portfolio-list sort-destination" data-sort-id="connectToFlyCompanyService" style="position: relative; height: 888px;">

                    @foreach($companyConnectToFlyServiceItems as $companyConnectToFlyServiceItem)
                        <div class="col-sm-6 col-lg-3 isotope-item {{ $companyConnectToFlyServiceItem->connectToFlyServiceCategory->slug }}" style="position: absolute; left: 0px; top: 0px;">
    ...
    <div class="container py-2 mt-5 mb-5">
        <div class="row mb-4">
            <div class="col text-center appear-animation" data-appear-animation="fadeInUpShorter" data-appear-animation-delay="200">
                <h2 class="font-weight-bold mb-2">{{ $connectToFlyHeadingAndSubText->headingConnectToFlyCompanyTeam }}</h2>
                <p class="opacity-7">{{ $connectToFlyHeadingAndSubText->subTextConnectToFlyCompanyTeam }}</p>
            </div>
        </div>
        <div class=" sort-destination-loader-showing mt-4 pt-2">
            <div class="row portfolio-list">

                @foreach($companyConnectToFlyTeams as $key => $companyConnectToFlyTeam)
                        ...
                        <span class="thumb-info thumb-info-swap-content thumb-info-centered-icons thumb-info-wrapper-direction-aware">
                            <span class="thumb-info-wrapper">
                                ...
                                <span class="thumb-info-title">
                                    ...
                                    <span class="thumb-info-type">{{ $companyConnectToFlyTeam->position }}</span>
                                </span>
                                        ...
                                        <div class="card-body py-3 px-2">
                                            <div class="testimonial testimonial-style-4 border-0 box-shadow-none m-0 p-0 text-start">
                                                <div class="testimonial-author pb-3">
                                                    <p><strong class="font-weight-normal text-light">{{ $companyConnectToFlyTeam->name }}</strong><span class="font-weight-normal text-light">{{ $companyConnectToFlyTeam->position }}</span></p>
                                                </div>
